...
async def draw_role_info(info_data: PropList):
    ...
    # 立绘
    gacha = await load_img(
        gacha_file / "_".join(["UI", "Gacha", "AvatarIcon", role.talent.abbr + ".png"])
    )
    await img.paste(
        image=gacha.resize((int(gacha.size[0] * 600 / gacha.size[1]), 600)), pos=(0, 0)
    )
    # 信息
    await img.text(
        role.name,
    ...
    await propcase.text(
        f"{int(role.fight_prop.elemental_mastery)}",
        width=(150, 300),
        height=(120, 160),
        align="right",
        font=Font().get(font="Yozai", size=25),
    ...
    await propcase.text(
        f"{round(role.fight_prop.crit_rate*100,1)}%",
        width=(150, 300),
        height=(160, 200),
        align="right",
        font=Font().get(font="Yozai", size=25),
    ...
    await propcase.text(
        f"{round(role.fight_prop.crit_dmg*100,1)}%",
        width=(150, 300),
        height=(200, 240),
        align="right",
        font=Font().get(font="Yozai", size=25),
    ...
    await propcase.text(
        f"{round(role.fight_prop.recharge*100,1)}%",
        width=(150, 300),
        height=(240, 280),
        align="right",
        font=Font().get(font="Yozai", size=25),
    ...
    await propcase.text(
        f"{round(role.fight_prop.get_elem_dmg(add_hurt)*100,1)}%",
        width=(150, 300),
        height=(280, 320),
        align="right",
        font=Font().get(font="Yozai", size=25),
    ...
    weapon_icon = await load_img(
        weapon_file / (role.weapon.icon + ".png"), size=(100, 100)
    )
    await weapon_case.paste(weapon_icon, pos=(0, 0))
    await weapon_case.text(
        f"{role.weapon.name}",
        width=(100, 250),
        height=10,
        align="center",
    ...
    for i, l in enumerate(["skill_A", "skill_E", "skill_Q"]):
        await skill_case.paste(
            await load_img(active_icon, size=(120, 120)), pos=(10 + i * 130, 0)
        )
        await skill_case.paste(
            await load_img(talent_file / role.talent.skill_icon[i], size=(80, 80)),
            pos=(30 + i * 130, 20),
        )
        await skill_case.text(
            role.talent.dict()[l],
            width=(52 + i * 135, 80 + i * 135),
            height=120,
    ...
    for i in range(6):
        if conste_flag[i]:
            await skill_case.paste(
                await load_img(active_icon, size=(80, 80)), pos=(10 + i * 60, 160)
            )
        else:
            await skill_case.paste(
                await load_img(inactive_icon, size=(80, 80)), pos=(10 + i * 60, 160)
            )
        await skill_case.paste(
            await load_img(
                conste_file / f"UI_Talent_{role.talent.abbr}_{str(i+1)}.png",
                size=(40, 40),
            ),
            pos=(30 + i * 60, 180),
        )
...
async def get_relic_case(item: Union[Relic, dict], score: Optional[float]):
    ...
    if isinstance(item, dict):
        ...
        await box.paste(
            await load_img(artifacts_file / item.icon, size=(80, 80)), pos=(0, 0)
        )
        await box.text(
            item.name, width=100, height=10, font=Font().get("Yozai", size=20)
        )
