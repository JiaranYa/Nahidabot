...
from Nahidabot.utils.classmodel import RoleInfo, Talent, FightProp, Weapon, PlayerInfo, Relic, Relicset, PropertySlot
...
class Player(Model):
    ...
    player_info =       fields.JSONField(encoder=PlayerInfo.json,
    ...
    async def insert_or_update(cls,uid:str,data:dict,user_qq:int):
        ...
        player.player_info = PlayerInfo(uid=uid,
...
class PropList(Model):
    ...
    talent =            fields.JSONField(encoder=Talent.json,
                                        decoder=Talent.parse_raw,
                                        description="等级，命座和天赋",
                                        null=True) 
    property =          fields.JSONField(encoder=FightProp.json,
                                        decoder=FightProp.parse_raw,
                                        description="面板数值",
                                        null=True)  
    weapon =            fields.JSONField(encoder=Weapon.json,
                                        decoder=Weapon.parse_raw,
                                        description="武器",
                                        null=True)
    artifacts =         fields.JSONField(encoder=Relicset.json,
                                        decoder=Relicset.parse_raw,
                                        description="圣遗物",
                                        null=True)
    dmg_info =          fields.JSONField(description="伤害数据",null=True)
    dmg_setting =       fields.JSONField(description="伤害设置信息",null=True)
    ...
    @classmethod
    async def inser_or_update_role(cls,uid:str,data_list:list[dict],user_qq:str):
        weapon_map = await load_json(STATIC_PATH / "weapon.json")
        artifact_map = await load_json(STATIC_PATH / "artifact.json")

        for data in data_list:
            role_info:RoleInfo
            [role_info,]= await RoleBasicInfo.filter(aid=str(data["avatarId"])).values_list("info",flat=True)
            role_name = role_info.name

            role,_ = await cls.get_or_create(user_qq=user_qq,uid=uid,role_name=role_name)
            # 等级，命座和天赋
            ...
            role.talent = Talent(level=int(data["propMap"]["4001"]["val"]),
                                element=elem_type,
                                fetter=data["fetterInfo"]["expLevel"],
                                weapon_type=role_info.weapon_type,
                                abbr=abbr,
                                constellation=len(data["talentIdList"])if "talentIdList" in data else 0,
                                skill_A=data["skillLevelMap"][str(skill_list[0])],
                                skill_E=data["skillLevelMap"][str(skill_list[1])],
                                skill_Q=data["skillLevelMap"][str(skill_list[2])],
                                skill_E_prod=3 if data.get("proudSkillExtraLevelMap",{}).get(proud_map.get(str(skill_list[1]))) else 0,
                                skill_Q_prod=3 if data.get("proudSkillExtraLevelMap",{}).get(proud_map.get(str(skill_list[2]))) else 0)
            # 面板数值
            props = data["fightPropMap"]
            role.property = FightProp(hp_base=props["1"],
                                    hp=props["2000"],
                                    atk_base=props["4"],
                                    atk=props["2001"],
                                    def_base=props["7"],
            ...
            for item in data["equipList"]:
                if item["flat"]["itemType"] == "ITEM_WEAPON":
                    role.weapon = Weapon(name=weapon_map[item["flat"]["nameTextMapHash"]],
                                        icon=item["flat"]["icon"],
                    ...
                    icon = item["flat"]["icon"]
                    main_stat = PropertySlot(name=item["flat"]["reliquaryMainstat"]["mainPropId"],
                                            value=item["flat"]["reliquaryMainstat"]["statValue"])
                    sub_stat_list=[]
                    for i in item["flat"]["reliquarySubstats"]:
                        sub_stat_list.append(PropertySlot(  name=i["appendPropId"],
                                                            value=i["statValue"]))

                    temp_relic = Relic(name=artifact_map["Name"][icon],
                                        ...
                                        rank=item["flat"]["rankLevel"])
                    if temp_relic.slot == "EQUIP_BRACER": relic_list.flower = temp_relic
                    elif temp_relic.slot == "EQUIP_NECKLACE": relic_list.plume = temp_relic
                    elif temp_relic.slot == "EQUIP_SHOES": relic_list.sands = temp_relic
                    elif temp_relic.slot == "EQUIP_RING": relic_list.goblet = temp_relic
                    elif temp_relic.slot == "EQUIP_DRESS": relic_list.circlet = temp_relic
